...
def _get_all_model_data(log_errors=True):
    ...
    try:
        ...
        if log_errors: print(f"\033[91m[PromptCrafter] Ollama connection failed. Is it running at {config.OLLAMA_BASE}? Error: {e}\033[0m")
    except requests.exceptions.RequestException as e:
        if log_errors: print(f"\033[93m[PromptCrafter] Warning: Could not fetch Ollama models. Error: {e}\033[0m")

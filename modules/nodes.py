...
# ------------------------------------------------------------------------------------
# PromptCraft Creator Nodes (Base, Image, Video, Lyrics)
# ------------------------------------------------------------------------------------
class PromptCraft_BaseCreator: # noqa
    ...
    def _setup_config(self, mode, user_text, model, images_with_weights=None, **kwargs): # noqa
        ...
        config_fields = {f.name for f in fields(config.PromptCraftRunConfig)}
        filtered_params = {k: v for k, v in config_params.items() if k in config_fields}

        run_config = config.PromptCraftRunConfig(**filtered_params)
...
{instructions}""")
    ...
    def _generate_prompt_for_scene(self, scene_text, mode, images_with_weights, image_context_for_all, style_rules, run_config, **kwargs): # noqa
        ...
        if config.CACHE.has(cache_key):
            print(f"\033[94m[PromptCrafter] Using cached prompt for scene: '{scene_text[:50]}...'")
            return config.CACHE.get(cache_key)
...
# ------------------------------------------------------------------------------------
# PromptCraft_FileOrganizer Node
# ------------------------------------------------------------------------------------
class PromptCraft_FileOrganizer:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
...
class PromptCraft_VideoCreator(PromptCraft_BaseCreator):
    @classmethod
    def INPUT_TYPES(cls):
        types = copy.deepcopy(PromptCraft_ImageCreator.INPUT_TYPES())
...
NODE_CLASS_MAPPINGS = {
    ...
    "PromptCraft_FileOrganizer": PromptCraft_FileOrganizer,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "PromptCraft_QnA": f"PromptCrafter QnA",
    "PromptCraft_Captioner": f"PromptCrafter Image Captioner",
    "PromptCraft_ImageCreator": f"PromptCrafter Image Prompt Creator",
    "PromptCraft_VideoCreator": f"PromptCrafter Video Prompt Creator",
    "PromptCraft_LyricsCreator": f"PromptCrafter Lyrics Creator",
    "PromptCraft_ClearCache": f"PromptCrafter Cache Utility",
    "PromptCraft_FileOrganizer": f"PromptCrafter File Organizer",
}
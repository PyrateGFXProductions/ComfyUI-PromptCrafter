...
- `PGFX_VRGDG_Bridge_V1.md`: exact bridge contract for feeding PromptCrafter planning outputs into the existing VRGDG LTX/Z-image workflow family without overwriting the current VRGDG workflows. Covers line-locking, slang normalization, story-group continuity metadata, prompt-map export, and validation.
- `PGFX_VRGDG_Slang_Glossary_Template_V1.txt`: paste-ready glossary template for song-specific slang or idioms. Feed it through `LoadText` into the bridge node's `custom_glossary_text`.

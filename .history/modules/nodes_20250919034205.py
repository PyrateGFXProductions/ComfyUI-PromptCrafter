...
class PromptCraft_BaseCreator:
    ...
    def _setup_config(self, mode, user_text, vision_model, **kwargs):
        ...
        config_params.update({'model': vision_model, 'language': utils._detect_language(user_text), 'temperature': temperature, 'use_chat_api': use_chat_api, 'max_length_words': max_length_words})
...
class PromptCraft_ImageCreator(PromptCraft_BaseCreator):
    ...
    def execute(self, user_text, vision_model, **kwargs):
        try:
            run_config = self._setup_config("Image", user_text, vision_model, **kwargs)
...
class PromptCraft_VideoCreator(PromptCraft_BaseCreator):
    ...
    def execute(self, user_text, vision_model, **kwargs):
        try:
            run_config = self._setup_config("Video", user_text, vision_model, **kwargs)
            images_with_weights = self._collect_images_with_weights(**kwargs)
            if kwargs.get("generate_schedule"):
...
class PromptCraft_LyricsCreator(PromptCraft_BaseCreator):
    ...
    def execute(self, user_text, vision_model, **kwargs):
        try:
            run_config = self._setup_config("Lyrics", user_text, vision_model, **kwargs)
            images_with_weights = self._collect_images_with_weights(**kwargs)

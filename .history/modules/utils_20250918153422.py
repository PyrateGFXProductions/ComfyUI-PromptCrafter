...
def _deep_think_and_refine(model, generation_prompt_text, max_iterations=3, confidence_threshold=0.8, **kwargs):
    ...
    critique_kwargs.pop('images', None); critique_kwargs.pop('images_b64', None); critique_kwargs.pop('debug_title', None)
    ...
    for i in range(max_iterations):
        ...
        if not current_prompt: # Only do initial generation if we have no prompt to start with
            ...
            reason_kwargs.pop('debug_title', None)
            ok, critique_json = api_clients._reason_with_model(model, initial_gen_template, **reason_kwargs, debug_title=f"Deep Think - Initial Generation & Critique")
        else:

...
class BaseAPIClient:
    ...
    def _make_request(self, url, headers, payload, timeout):
        ...
        try:
            ...
            return False, f"{self.provider.capitalize()} API Error: Could not connect to {e.request.url}. Please check the server address and ensure it is running."
        except config.requests.exceptions.Timeout as e:
            return False, f"{self.provider.capitalize()} API Error: The request timed out after {timeout} seconds. The server may be overloaded or slow to respond."
            ...
            return False, f"{self.provider.capitalize()} API Error: {error_details}"
        except json.JSONDecodeError:
...
class OllamaClient(BaseAPIClient):
    ...
    def query(self, model_id, prompt, images_b64=None, timeout=60, temperature=None, seed=None, prefer_chat=False, **kwargs):
        ...
        for mode in order:
            ...
            ok, data_or_err = self._make_request(url=f"{self.base_url}/api/{mode}", headers={}, payload=payload, timeout=timeout)
            if ok:
                ok_parse, text = self._parse_response(data_or_err)
                if ok_parse: return True, text.strip()
                last_err = text
            else:
                last_err = data_or_err # This will now be a more specific error message
                if "Could not connect" in str(last_err): break # Stop retrying if we can't even connect
    ...
    def _parse_response(self, data):
        ...
        return False, f"Could not find response content in Ollama output: {json.dumps(data)}"
...
def _get_models_by_type(model_type):
    ...
    if ollama_status == 'ok':
        for m in ollama_models_details:
            ...
            is_vision = any(f in VISION_FAMILIES for f in families) or 'clip' in families or 'vision' in architecture
    ...
    if not available_models:
        return ["OLLAMA_UNREACHABLE_CHECK_SERVER"] if ollama_status == 'connection_error' else ["NO_MODELS_FOUND_OR_CONFIGURED"]

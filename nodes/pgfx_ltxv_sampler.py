    ...
    for item in cond:
        if isinstance(item, (list, tuple)) and len(item) >= 2 and not isinstance(item, dict):
            ...
            if ctx is None:
                ...
                if tensors:
                    ctx = sorted(tensors, key=lambda x: x[0], reverse=True)[0][1]
            if ctx is None:
    ...
    def sample(
        ...
        if (
            ...
            if not _is_nested_latent(guiding_latents):
                print(
                    "### [PGFX_LTXVInContextSampler] Auto-wrapping guiding_latents with silent audio for LTXAV model",
                    file=sys.stderr,
                )
                av_model = guider.model_patcher.model.diffusion_model
                guiding_latents = guiding_latents.copy()
                guiding_latents["samples"] = NestedTensor((
                    guiding_latents["samples"],
                    torch.zeros(
                        (guiding_latents["samples"].shape[0], av_model.num_audio_channels, guiding_latents["samples"].shape[2], av_model.audio_frequency_bins),
                        device=guiding_latents["samples"].device,
                        dtype=guiding_latents["samples"].dtype,
                    ),
                ))
